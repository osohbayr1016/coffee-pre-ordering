  ...
  return (
    <div>
      ...
      <div className="glass-panel p-6 mb-8 border border-white/10">
        <h2 className="text-lg font-semibold mb-4">Global Modifiers</h2>
        <div className="flex gap-4">
          <div className="flex items-center gap-3 bg-white/5 p-3 rounded-lg flex-1 justify-between">
            <span>Oat Milk</span>
          ...
          <div className="flex items-center gap-3 bg-white/5 p-3 rounded-lg flex-1 justify-between opacity-50">
            <span>Almond Milk</span>
            <button className="w-10 h-5 bg-zinc-600 rounded-full relative transition-colors">
              <span className="absolute left-1 top-0.5 w-4 h-4 bg-white rounded-full"></span>
            </button>
          </div>
        </div>
        <p className="text-xs text-white/40 mt-3">Toggling these off will immediately hide them from all customer menus globally.</p>
      </div>
            ...
            {menu.map((item) => (
              <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                ...
                <td className="p-4 text-right">
                  <button className="text-white/40 hover:text-white transition-colors text-sm underline mr-3">Edit</button>
                  <button className="text-red-400 hover:text-red-300 transition-colors text-sm underline">Hide</button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className="glass-panel w-full max-w-md p-6 border border-white/20">
            ...
            <form onSubmit={handleAddItem} className="space-y-4">
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <input 
                    ...
                    className="w-full bg-black/50 border border-white/20 rounded-lg p-2 text-white"
                  />
                  <p className="text-[10px] text-white/40 mt-1">Unique preparation time</p>
                </div>
                <div className="flex-1">
              ...
              <div>
                ...
                <input 
                  type="url" value={newItem.image_url} onChange={e => setNewItem({...newItem, image_url: e.target.value})}
                  className="w-full bg-black/50 border border-white/20 rounded-lg p-2 text-white" placeholder="https://..."
                />
              </div>
              ...
              <div className="flex justify-end gap-3 mt-8">
                ...
                <button 
                  ...
                  className="bg-coffee hover:bg-coffee-light px-6 py-2 rounded-lg font-medium text-white transition-colors"
                >
